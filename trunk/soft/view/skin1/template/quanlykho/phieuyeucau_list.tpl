...
<div id="ben-search"><label>Ngày lập từ ngày</label> <script
	language="javascript">
                    $(function() {
                        $("#tungay").datepicker({
                                changeMonth: true,
	...
	value="" class="text" /> <label>đến ngày</label> <script
	language="javascript">
                $(function() {
                    $("#denngay").datepicker({
                            changeMonth: true,
	...
	value="" class="text" /> <label>Tình trạng phiếu</label> <select
...
<input type="button" class="button" name="btnSearch" value="Tìm"
	...
	onclick="window.location = '?route=quanlykho/phieuyeucau'" /></div>
<!-- end search -->
	...
	foreach($datas as $key => $item)
	{
		...
		<tr>
			<td class="check-column"><input class="inputchk" type="checkbox"
				...
				value="<?php echo $item['maphieu']?>"></td>
			...
			<td class="link-control"><input type="button" class="button"
				...
				type="button" class="button" name="btnPhanHoi"
				value="<?php echo $item['text_phanhoi']?>"
				onclick="window.location='<?php echo $item['link_phanhoi']?>'" /></td>
		</tr>
		<?php
	}
	?>
...
function deleteitem()
{
	...
	if (answer)
	{
		$.post("?route=quanlykho/phieuyeucau/delete", 
				$("#listitem").serialize(), 
				function(data)
				{
					if(data!="")
...
function searchForm()
{
	var url =  "?route=quanlykho/phieuyeucau";
	if($("#tungay").val() != "")
