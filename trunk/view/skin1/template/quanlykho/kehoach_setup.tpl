...
$('#listkehoachsanpham').load("?route=quanlykho/kehoach/loadKehoachSanPham&id=<?php echo $item['id']?>",function(){
		numberReady();
	});
function save()
{
	...
	$.post("?route=quanlykho/kehoach/savechitietkehoach", $("#frm").serialize(),
		function(data){
			var arr = data.split('-');
			if(arr[0] == "true")
			{
