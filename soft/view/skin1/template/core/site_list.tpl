	...
	<tbody>
		...
		<tr>
			<td class="check-column"><input class="inputchk" type="checkbox"
				name="delete[<?php echo $site['siteid']?>]"
				value="<?php echo $site['siteid']?>"></td>
			<td><?php echo $site['siteid']?></td>
			<td><?php echo $site['sitename']?></td>
			<td><?php echo $site['status']?></td>

			<td class="link-control"><a class="button"
				href="<?php echo $site['link_edit']?>"
				title="<?php echo $site['text_edit']?>">Edit</a> <?php if($site['siteid']!="default"){ ?>
			<a class="button" onclick="copySite('<?php echo $site['siteid']?>')">Copy
			default</a> <?php }?></td>
		</tr>
		<?php
		}
